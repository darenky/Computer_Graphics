...
embedded_image = embed_watermark('img.png', watermark_binary, 8)  # Specify the bit plane (1-8)
embedded_image.save('embedded_watermark_image.png')

def text_to_bits(text):
    return ''.join(format(ord(char), '08b') for char in text)

def embed_text(input_image_path, text):
    image = Image.open(input_image_path)
    image_array = np.array(image)
...
embedded_text_image = embed_text('embedded_watermark_image.png', "Hidden text here")
embedded_text_image.save('8bits.png')
